...
function sl_pred(params, dat=sl_dat)
    a = params[1]
    b = params[2]
    c = params[3]
    t₀ = params[4]
...
optim_out = optimize(lik_quadratic, [2.0, 10.0, -250.0, 1800.0], BFGS())
...
resids = dat[:, 2] - sl_pred(optim_out.minimizer)
...
boot_resids = gen_ar1_resids(ρ, σ², nrow(sl_dat), n_boot)
...
for i = 1:nrow(boot_params)
    optim_boot = optimize(p -> lik_quadratic(p, hcat(sl_dat[:, 1], boot_slr[i, :])), [0.0, 0.0, -100.0, 1800.0]; iterations=50000)
